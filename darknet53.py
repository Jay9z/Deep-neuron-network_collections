...
class Darknet53(nn.Module):
	...
	def forward(self, entry):
		...
		return conv45_52, conv28_43, conv11_26		# YOLOv3用，所以输出了3

if __name__ == "__main__":
	import cv2
	import torch
	img = cv2.imread("test1.jpg",cv2.IMREAD_UNCHANGED)/255.0*2-1
	input = torch.tensor([img]).permute(0,3,1,2).float()

	net = Darknet53()
	result = net(input)
	for i,output in enumerate(result):
		print(f"result {i} with shape: ", output.shape)
		print(output)
